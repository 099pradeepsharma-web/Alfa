...
// New helper to format the AI report text into readable HTML
const formatReport = (text: string) => {
    // Split into lines and handle markdown-like syntax
    const parts = text.split('\n').map((line, index) => {
        // Match bolded headings like **Overall Summary:** or 1. **...**
        if (line.match(/^\s*\d*\.?\s*\*\*(.+?):\*\*/)) {
    ...
    return <div className="prose prose-sm max-w-none dark:prose-invert prose-headings:text-slate-800 dark:prose-headings:text-slate-100 prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-li:text-slate-600 dark:prose-li:text-slate-300">{parts}</div>;
};
...
const StudentPerformanceView: React.FC<StudentPerformanceViewProps> = ({ userRole, student, language, onBack }) => {
    ...
    const [isAnalyzing, setIsAnalyzing] = useState<string | null>(null); // store questionId being analyzed
    ...
    useEffect(() => {
        const fetchPerformanceData = async () => {
            if (activeTab === 'performance' || activeTab === 'studyPatterns') {
    ...
    }, [student.id, student.performance, activeTab]);
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            if (activeTab === 'questions') {
                ...
                try {
                    ...
                    setError(err.message || t('errorFetchingQuestions'));
                } finally {
        ...
        fetchQuestions();
    }, [activeTab, student.id, language, t]);
    ...
    const handleGenerateReport = async () => {
        ...
        setReport(null);
        setError(null);
        setIsReportFromDB(false);
        setFeedbackRating(null);
        setFeedbackComment('');
        setFeedbackSubmitted(false);

        try {
            const studentWithFullPerformance = { ...student, performance: allPerformance };
            const cachedReport = await getReport(student.id, userRole, language);
            if (cachedReport) {
                setReport(cachedReport);
                setIsReportFromDB(true);
                return;
            }

            const generatedReport = userRole === 'teacher'
                ? await generateTeacherReport(studentWithFullPerformance, language)
                : await generateParentReport(studentWithFullPerformance, language);
            
            setReport(generatedReport);
            await saveReport(student.id, userRole, generatedReport, language);
    ...
    const handleFeedbackSubmit = async () => {
        if (!feedbackRating) return; // Rating is required
        ...
        const feedbackData: AIFeedback = {
            ...
            contentIdentifier: reportIdentifier,
            rating: feedbackRating,
            comment: feedbackComment,
            timestamp: new Date().toISOString(),
        };
        try {
            await saveAIFeedback(feedbackData);
    ...
    const handleGeneratePracticeSheet = async () => {
        ...
        try {
            if (allPerformance.length === 0) {
                setError(t('noPerformanceDataError'));
                return;
            }
    ...
    const chartData = useMemo(() => {
        if (activeTab !== 'studyPatterns') return null;

        // 1. Weekly Activity Heatmap
        const activityByDate: { [key: string]: number } = {};
        allPerformance.forEach(rec => {
        ...
        allPerformance.forEach(rec => {
            if (rec.type === 'quiz' || rec.type === 'exercise') {
                if (!subjectData[rec.subject]) {
                    subjectData[rec.subject] = { totalScore: 0, count: 0 };
                }
        ...
        // 3. Learning Style
        const learningStyle = allPerformance.reduce((acc, rec) => {
    ...
    }, [allPerformance, activeTab]);

    
    if (practiceSheet) {
        return <Quiz questions={practiceSheet} onBack={() => setPracticeSheet(null)} chapterTitle={t('practiceSheet')} />;
    }
    ...
    const renderTabContent = () => {
        switch(activeTab) {
            case 'performance':
                return (
                    <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
                       {allPerformance.length > 0 ? allPerformance.map((record, index) => (
            ...
            case 'studyPatterns':
                 if (!chartData) return <div className="flex justify-center py-8"><LoadingSpinner /></div>;
                 return (
            ...
            case 'questions':
                return (
                    <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                        {isLoadingQuestions ? <div className="flex justify-center py-8"><LoadingSpinner /></div> : 
                         questions.length > 0 ? questions.map(q => (
                             <div key={q.id} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                                ...
                                <p className="text-sm text-slate-600 dark:text-slate-300">
                                    {t('context')}: {tCurriculum(q.chapter)} &gt; <span className="font-medium">{tCurriculum(q.concept)}</span>
                                </p>

                                {q.fittoResponse && (
                                    <div className="mt-4 bg-slate-200 dark:bg-slate-700 p-3 rounded-lg">
                                        <h5 className="font-bold text-sm text-slate-600 dark:text-slate-200 flex items-center">
                                            <SparklesIcon className="h-4 w-4 mr-1.5 text-primary" style={{color: 'rgb(var(--c-primary))'}} />
                                ...
                                {analysis[q.id] ? (
                                    <div className="mt-4 space-y-3 animate-fade-in">
                                        <div className="bg-primary-light/60 dark:bg-slate-900/50 p-3 rounded">
                                            <h5 className="font-bold text-sm text-primary-dark" style={{color: 'rgb(var(--c-primary-dark))'}}>{t('modelAnswer')}</h5>
                                            <p className="text-sm text-slate-700 dark:text-slate-300">{analysis[q.id]?.modelAnswer}</p>
                                        </div>
                                        <div className="bg-primary-light/60 dark:bg-slate-900/50 p-3 rounded">
                                            <h5 className="font-bold text-sm text-primary-dark" style={{color: 'rgb(var(--c-primary-dark))'}}>{t('pedagogicalNotes')}</h5>
                                            <p className="text-sm text-slate-700 dark:text-slate-300">{analysis[q.id]?.pedagogicalNotes}</p>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="mt-3 text-right">
                                        <button 
                                            onClick={() => handleGetAnalysis(q)}
                                            disabled={isAnalyzing === q.id}
                                            className="px-3 py-1.5 text-xs font-semibold bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-600 transition shadow-sm disabled:opacity-50 flex items-center"
                                        >
                ...
                return (
                    <div className="space-y-4">
                       <button 
                            ...
                            className="w-full flex items-center justify-center px-4 py-3 text-white font-bold rounded-lg btn-primary-gradient disabled:opacity-70 disabled:cursor-not-allowed"
                        >
                       ...
                       {userRole === 'teacher' && (
                            <button 
                                onClick={handleGeneratePracticeSheet}
                                disabled={isGeneratingSheet}
                                className="w-full flex items-center justify-center px-4 py-3 bg-teal-600 text-white font-bold rounded-lg shadow-md hover:bg-teal-700 transition disabled:bg-teal-400 disabled:cursor-not-allowed"
                            >
                       ...
                       {report && (
                            <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg animate-fade-in max-h-[450px] overflow-y-auto">
                                <div className="flex justify-between items-start mb-2">
                                    <h4 className="font-bold text-slate-800 dark:text-slate-100 text-lg flex items-center">
                                        <SparklesIcon className="h-5 w-5 mr-2 text-primary" style={{color: 'rgb(var(--c-primary))'}}/>
                                        {t('aiGeneratedReport')}
                                    </h4>
                                    {isReportFromDB && (
                                        <div className="flex items-center bg-teal-100 dark:bg-teal-900/50 text-teal-800 dark:text-teal-300 text-xs font-medium px-2 py-0.5 rounded-full">
                        ...
                        {report && !isGeneratingReport && (
                            <div className="mt-6 pt-4 border-t border-dashed border-slate-200 dark:border-slate-700">
                                {feedbackSubmitted ? (
                                    <div className="text-center p-4 bg-green-50 dark:bg-green-900/30 rounded-lg text-green-700 dark:text-green-300 font-semibold animate-fade-in">
                                        {t('feedbackThanks')}
                                    </div>
                                ) : (
                                    <div className="animate-fade-in">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => setFeedbackRating('up')} className={`p-2 rounded-full transition ${feedbackRating === 'up' ? 'bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-300' : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600'}`}>
                                                {feedbackRating === 'up' ? <HandThumbUpSolid className="h-6 w-6" /> : <HandThumbUpIcon className="h-6 w-6" />}
                                            </button>
                                            <button onClick={() => setFeedbackRating('down')} className={`p-2 rounded-full transition ${feedbackRating === 'down' ? 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-300' : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600'}`}>
                                                {feedbackRating === 'down' ? <HandThumbDownSolid className="h-6 w-6" /> : <HandThumbDownIcon className="h-6 w-6" />}
                                            </button>
                                            <textarea
                                                value={feedbackComment}
                                                onChange={(e) => setFeedbackComment(e.target.value)}
                                                placeholder={t('commentOptional')}
                                                className="flex-grow p-2 text-sm border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900/50 rounded-md focus:ring-1 focus:ring-primary"
                                                rows={1}
                                            />
                                        </div>
                                        {feedbackRating && (
                                            <div className="text-right mt-3">
                                                <button onClick={handleFeedbackSubmit} className="px-4 py-1.5 text-sm bg-primary text-white font-semibold rounded-lg shadow-sm hover:bg-primary-dark transition" style={{backgroundColor: 'rgb(var(--c-primary))'}}>
                                                    {t('submitFeedback')}
                                                </button>
                                            </div>
                                        )}
                ...
                {days.map(date => {
                    ...
                    return (
                        <div 
                            ...
                            title={`${date.toLocaleDateString()}: ${count} activities`}
                        />
                    );
                })}
            </div>
